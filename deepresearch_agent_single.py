# -*- coding: utf-8 -*-
"""Single-file Deep Research Agent for vLLM + BrowseComp-Plus."""
    ...
    def __init__(self, index_path: str | Path) -> None:
        ...
        if not self.index_path.exists():
            raise FileNotFoundError(
                ...
                "Please build it first with the provided index builder."
            )
...
FINAL_ANSWER_PROMPT = (
    "If the facts above are sufficient to answer, write the answer. "
    "If not, write: Unable to determine.\n\nAnswer:"
)
...
def _chat(client, model: str, msgs: List[Dict[str, Any]], max_tok: int = 2048) -> str:
    try:
...
def _parse_dead_ends(text: str) -> List[str]:
    dead = _parse_section(text, "Dead Ends")
    clean = []
    for item in dead:
        low = item.lower().strip()
        if low in ("none", "none.", "(none)", "(none if nothing)", "...", ""):
...
def _parse_status(text: str) -> str:
    m = re.search(r"Status:\s*\*{0,2}\s*(YES|NO)\s*\*{0,2}", text, re.I)
...
def _fallback_query(question: str, memory: AgentMemory) -> str:
    ...
    for i in range(len(words) - 1):
        ...
        if w1 not in stop or w2 not in stop:
...
class ResearchContext:
    """Per-agent context: each step sees only what it needs."""

    def __init__(self, question: str, memory: AgentMemory):
        self.q = question
        self.m = memory

    def _trim(self, parts: List[str]) -> str:
        result = "\n".join(parts)
        facts = self.m.facts
    ...
    def for_assessor(self, current_query: str = "") -> str:
        extra = f"Most recent query: \"{current_query}\"\nRounds searched: {len(self.m.searched_queries)}"
        return self._trim(
            [
                f"## Question\n{self.q}",
                f"\n## Facts\n{self.m.facts_summary()}",
...
def _step_search(
    ...
    query: str,
    memory: AgentMemory,
    searcher: Optional[BrowseCompBM25Searcher] = None,
) -> List[Dict[str, Any]]:
...
def _step_screen(client, model: str, question: str, results: List[Dict[str, Any]], memory: AgentMemory) -> str:
    ctx = ResearchContext(question, memory).for_screen()
    prompt = f"{ctx}\n\n## Search Results\n{_format_results(results)}\n\n{DOC_SCREEN_PROMPT}"
    msgs = [{"role": "system", "content": SYSTEM_SCREEN}, {"role": "user", "content": prompt}]
    return _chat(client, model, msgs, max_tok=1024)


def _step_fetch(registry: Dict[str, Any], docids: List[str], memory: AgentMemory) -> List[Dict[str, Any]]:
    ...
    for did in docids[:3]:
        ...
        if doc is None or (isinstance(doc, dict) and "error" in doc):
            ...
            docs.append({"docid": did, "text": doc.get("text", ""), "url": doc.get("url", "")})
    return docs


def _step_extract(client, model: str, question: str, docs: List[Dict[str, Any]], memory: AgentMemory) -> str:
    if not docs:
        return ""
    ctx = ResearchContext(question, memory).for_executor()
    prompt = f"{ctx}\n\n## Full Documents\n{_format_docs(docs)}\n\n{FACT_EXTRACT_PROMPT}"
    msgs = [{"role": "system", "content": SYSTEM_EXECUTOR}, {"role": "user", "content": prompt}]
    return _chat(client, model, msgs, max_tok=2048)


def _step_assess(client, model: str, question: str, memory: AgentMemory, current_query: str) -> str:
    ctx = ResearchContext(question, memory).for_assessor(current_query)
    prompt = f"{ctx}\n\n{PROGRESS_PROMPT}"
    msgs = [{"role": "system", "content": SYSTEM_ASSESSOR}, {"role": "user", "content": prompt}]
    return _chat(client, model, msgs, max_tok=2048)


def _step_final_answer(client, model: str, question: str, memory: AgentMemory) -> str:
    ctx = ResearchContext(question, memory).for_synthesizer()
    prompt = f"{ctx}\n\n{FINAL_ANSWER_PROMPT}"
    msgs = [{"role": "system", "content": SYSTEM_SYNTHESIZER}, {"role": "user", "content": prompt}]
    return _chat(client, model, msgs, max_tok=2048)


def _step_rethink(client, model: str, question: str, memory: AgentMemory) -> str:
                ...
                "tool_calls": [
                    {
                        "id": f"call_{call_id}",
                        "type": "function",
                        "function": {"name": "search", "arguments": json.dumps({"query": rec["query"]}, ensure_ascii=False)},
                    }
                ],
            }
        )
                    ...
                    [
                        {
                            "docid": d["docid"],
                            "score": d.get("score", 0),
                            "snippet": (d.get("snippet") or d.get("text", "") or "")[:300],
                        }
            ...
            messages.append(
                {
                    ...
                    "tool_call_id": f"call_{call_id}",
                    "content": json.dumps(
                        {"docid": d["docid"], "text_preview": (d.get("text", "") or "")[:500], "url": d.get("url", "")},
                        ensure_ascii=False,
                    ),
                }
            )
